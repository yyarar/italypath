...
Proxy kapsami (2026-09-26, denetim S9#6): `config.matcher` yalniz korumali sayfa oneklerini (`PROTECTED_PAGE_ROUTES` ile ayni liste), `/api/*` ve kanonik olmayan okul adreslerini (`/universities/003`, `%33`, `abc`) alir. Herkese acik sayfalar, kok dosyalar (`/llms.txt`, `/robots.txt`, `/sitemap.xml`), `/data/*` ve kanonik okul/program sayfalari proxy'siz sunulur (Vercel'de her proxy calismasi Active CPU harcar). Sunucuda `auth()` yalniz matcher'daki yollarda calisir (bugun yalniz `/api/sat/questions`). Oturumsuz istek davranisi (`proxy.ts` dal sirasi): public -> gecer; `PROTECTED_PAGE_ROUTES` -> `/giris?redirect_url=...`; `/api/*` -> Clerk'in varsayilan API cevabi; listede olmayan sayfa proxy'ye girmez ve Next'in 404 sayfasini alir (2026-09-25 ile 2026-09-26 arasi `/giris`'e gidiyordu; kod dali savunma olarak duruyor). Yeni korumali sayfa `PROTECTED_PAGE_ROUTES`'a ve `config.matcher`'a birlikte eklenir; `check:routes` esligi, envanteri ve kanonik okul adresi kapsamini dener. Korumali API handler'lari proxy'ye tek basina guvenmez: `/api/sat/questions` kendi `auth()` kontrolunu yapar ve oturum yoksa 401 doner; `check:routes` her korumali API icin bunu zorlar. Herkese acik webhook (`/api/webhooks/clerk`) oturum tasimaz; tek koruma Clerk imzasidir (`verifyWebhook`, `@clerk/nextjs/webhooks`). `check:routes`, `/api/webhooks/` altindaki her public handler'in baska isten once imzayi dogruladigini ve yalniz POST sundugunu zorlar.
...
Ana sayfa (`homeTools.sat.meta`, `VelocityBridge` -> `t.homeNumbers`) ve `/sat` basligi soru sayisini tek sabitten yazar: `lib/translations/tr.ts` + `en.ts` basindaki `SAT_QUESTION_COUNT_CLAIM` (`t.sat.questionCount`; TR "1.000+", EN "1,000+"; 2026-09-26, STATUS #64; canli bank 1.019 Math sorusu, ifade Kerem kararıyla sabit). Her SAT importundan sonra bu sabiti canli sayimla karsilastir. `VelocityBridge` metinleri `homeNumbers` altinda, bolge sayisi sunucudan (`app/page.tsx`, `SCHOLARSHIP_REGIONS.length`) prop olarak gelir; `components/home/HomeToolsSection.tsx` burs karti da ayni prop'u `homeTools.scholarships.meta` `{count}` yer tutucusuyla yazar (STATUS #91). Ortak program sitesi izin listesinin (`PARTNER_DOMAINS_BY_DEPARTMENT`) insan gozuyle gozden gecirme kanit tablosu `docs/reviews/2026-09-26-ortak-program-siteleri.md` (STATUS #62).
...
SAT yazicilari (`scripts/sat/import-bank.mjs`, `scripts/sat/patch-sat-questions.mjs`) da kabul dosyasi yazicilariyla ayni ortak katmani (`scripts/lib/program-details-import.mjs`: `parseImportArgs`, `createImportClient`, `assertTarget`) kullanir (2026-09-26, STATUS #65): bayraksiz calistirma kuru calistirmadir, yazma yalniz `--apply --project-ref kskbnxxyviowmrlskwke` ve `.env.local` adresi bu projeye aitse; `test:sat-patch` sahte istemciyle kanitlar. `scripts/sat/import-explanations.mjs` ve `import-authored-explanations.mjs` de ayni ortak katmani kullanir (2026-09-26, STATUS #92): okuma once `SUPABASE_SECRET_KEY` (yazmada yoksa `SUPABASE_SERVICE_ROLE_KEY`), `import-explanations.mjs` yalniz dogrudan calistirilinca calisir (isMain); `test:sat-explanations` sahte istemci + sentetik paketle kanitlar.

Olu link yeniden kontrolu (2026-09-26, STATUS #61): `node scripts/fix-dead-program-links-2026-09-26.mjs` 5 programin (897, 898, 901, 1074, 1291) 3 link alanini tarayici benzeri basliklarla canli kontrol eder (salt okuma; rapor `tmp/uni-research/`), `--replacements <json>` verilirse duzeltme plani kurar, `--apply --project-ref kskbnxxyviowmrlskwke` ile ortak katmandan yazar. 26 Eylul kontrolu: 15/15 adres 200; denetimin 'olu' bulgusu buyuk olasilikla bot engeli (WAF 403) yanlis pozitifiydi.
...
20. `/llms.txt` gibi kok dosyalar 2026-09-26'dan beri proxy'ye hic girmez (matcher yalniz korumali yollar, API ve kanonik olmayan okul adresi). Yine de public olacak her kok dosya `proxy.ts` allowlist'ine ve `scripts/check-route-access.mjs` public listesine birlikte eklenir (matcher genisletilirse erisim kaybolmasin); icerik guard'i tek basina erisimi kanitlamaz.
