...
| 53 | Güvenlik kartı 5 yayını (denetim S3#3, S2#2, S2#4, G4#1). Dal `claude/competent-pike-e3c652`, commit edildi, push edilmedi. **Yapıldı:** gönüllü masa Realtime okuma kuralları canlıda (2026-09-25 22:04 UTC, migration `volunteer_mentor_private_realtime`, Kerem onayı, önce 21:11 UTC yedeği `--verify` ile doğrulandı). Geri alınan canlı denemede gerçek 2 öğrenci + operatör kimliğiyle 10/10 giriş kararı doğru; öğrenci ve anonim yazma 42501 ile reddedildi. Anonimin Supabase'e ait yazma yetkisi kaldırılamadı, yazmayı RLS reddediyor. Canlıdaki herkese açık kanallar etkilenmez. Uzman formu şüpheli eşiği + 150 tavan da canlıda (2026-09-25 22:20 UTC, migration `expert_leads_suspected_threshold`, Kerem onayı, push öncesi). Geri alınan canlı denemede ilk 50 `new`, 51-150 `suspected` (100), 151. `expert_lead_rate_limited`, aynı gönderim tekrarı 23505; deneme satırı kalmadı. **Kalan sıra:** (1) dalı main'e birleştirip push (Kerem), (2) `/ekip/uzman` kontrolü, (3) iki hesaplı Realtime testi, (4) Supabase Realtime "Allow public access" kapatma, (5) testi tekrar. Panel ayarı push'tan önce kapatılırsa masa canlı güncellemeyi kaybeder. Ayrıntı `SUPABASE_SECURITY_RUNBOOK.md` "Private Realtime" | `supabase/expert_leads.sql`, `supabase/volunteer_mentor.sql`, `npm run test:mentor-db` | Kerem: push, test, panel ayarı |
