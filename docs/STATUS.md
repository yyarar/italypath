...
| --- | --- | --- | --- |
| 1 | Supabase Free egress mühleti: dönem 27 Eylül'de sıfırlanır (kapanış 8,04/5 GB), 14 Ekim'e kadar hedef < 1 GB; ikinci mühlet yok. 16-25 Eylül günlük ~20-65 MB, ama hedefli kabul çekimi 250 → 604/gün artıyor. 2026-09-26: düzeltme dalda (#54): program sayfası yalnız kendi kabul satırını, okul sayfası hiç kabul dosyası çekmez. 14 Ekim'den önce yayına girmeli; yayından sonra Usage ekranında günlük egress kontrol edilir | `docs/USAGE_LIMITS.md` Supabase; `SEO_AUDIT.md` §20.6; `AGENT_CONTEXT.md` "Veri Katmani" | Kerem: push zamanı |
...
| 54 | Güvenlik kartı 2 (denetim S9#2, O1#1, S9#3, S9#1 kod tarafı, S9#4, S4#5 anahtar tipi, O4#4, G4#2, O2#6, O4#2, S5#3, O1#6) 2026-09-26'da main'e birleştirildi ve push edildi (kart 5 ile aynı yayın): katalog okumaları server-only `SUPABASE_SECRET_KEY` (`sb_secret_`) ile; okul sayfası dizinden, program sayfası yalnız kendi kabul satırı; kanonik okul adresi (`/universities/003` → 308); Supabase istek süre sınırı + bayat memo; ISR sayfaları veri hatasını yakalamaz, markalı `app/global-error.tsx`; JSON-LD kaçışı; okul fotoğrafları Vercel görsel optimizasyonu dışında (`remotePatterns` kaldırıldı); sitemap 3 saat. Push öncesi Vercel Production `SUPABASE_SECRET_KEY` değerinin `sb_secret_` ile başladığı doğrulandı (2026-09-26, yalnız baş ek okundu). Yayın sonrası: okul/program sayfası, `/universities/003` yönlendirmesi ve okul fotoğrafları canlıda bir kez kontrol; ardından kart 4 (katalog tablosu yetkileri, Kerem onayıyla). S2#3 (`/api/universities` CDN önbelleği) Kerem kararıyla yapılmadı (2026-09-26) | `lib/universities.server.ts`, `lib/universityPath.ts`, `proxy.ts`, `AGENT_CONTEXT.md` "Veri Katmani" | Kerem: kart 4 onayı |
