...
Tarih: 2026-09-25 · Doğrulandığı commit: `771b995` · 2026-09-25: servis planları/kullanım (#1, #18, #21, #40-42) eklendi, kaynak `docs/USAGE_LIMITS.md` · 2026-09-25: Supabase yedek ve geri yükleme yolu (G2#2) kapandı, #43-45 eklendi · 2026-09-26: güvenlik kartı 2 (veri katmanı + egress) dalda, #1 güncellendi, #54 eklendi
...
| 54 | Güvenlik kartı 2 (denetim S9#2, O1#1, S9#3, S9#1 kod tarafı, S9#4, S4#5 anahtar tipi, O4#4, G4#2, O2#6, O4#2, S5#3, O1#6) dalda, push bekliyor: katalog okumaları server-only `SUPABASE_SECRET_KEY` (`sb_secret_`) ile; okul sayfası dizinden, program sayfası yalnız kendi kabul satırı; kanonik okul adresi (`/universities/003` → 308); Supabase istek süre sınırı + bayat memo; ISR sayfaları veri hatasını yakalamaz, markalı `app/global-error.tsx`; JSON-LD kaçışı; okul fotoğrafları Vercel görsel optimizasyonu dışında (`remotePatterns` kaldırıldı); sitemap 3 saat. Push öncesi: Vercel `SUPABASE_SECRET_KEY` değerinin `sb_secret_` ile başladığı doğrulanmalı (yoksa build düşer, yayın olmaz). Yayın sonrası: okul/program sayfası, `/universities/003` yönlendirmesi ve okul fotoğrafları canlıda bir kez kontrol; ardından kart 4 (katalog tablosu yetkileri, Kerem onayıyla). S2#3 (`/api/universities` CDN önbelleği) Kerem kararıyla yapılmadı (2026-09-26) | `lib/universities.server.ts`, `lib/universityPath.ts`, `proxy.ts`, `AGENT_CONTEXT.md` "Veri Katmani" | Kerem: push, anahtar kontrolü, kart 4 onayı |
