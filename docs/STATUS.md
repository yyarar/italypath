...
| 59 | Güvenlik kartı 7 (denetim S1#3, O4#5, O3#5, S7#5, O4#3, S5#5) dalda, push bekliyor: favori, belge, profil ve SAT kancaları Clerk'in yerel oturum anahtarına geçti (eski `supabase` JWT şablonu kodda yok); istekler 15 sn ile sınırlı, bağlantı hatasında boş liste yerine "yüklenemedi, tekrar dene" görünür (`/hosgeldin` kayıtlı cevaplar okunamazsa sihirbazı açmaz); girişsiz ziyaretçi Supabase kütüphanesini indirmez; belge linki yalnız "Görüntüle"ye basılınca 90 sn'lik üretilir, yeni yüklemeler önbelleğe alınmaz (eski dosyalar yükleme anındaki 1 saatlik ayarı taşır); SAT ilerlemesi soru başına son deneme + tek satırlık gün özeti ile okunur (`supabase/sat_progress.sql`); tarayıcı hafızası engelliyse sayfalar çökmez. Sıra: 1) SAT SQL'i canlıya (Kerem onayı; push'tan önce, yoksa yeni kod SAT ilerlemesini yükleyemez) 2) push 3) iki hesaplı test (runbook bölüm 5) 4) Clerk'te `supabase` JWT şablonunun imza ayarı not edilir ve şablon silinir (runbook bölüm 1) | `lib/useUserSupabaseClient.ts`, `supabase/sat_progress.sql`, `SUPABASE_SECURITY_RUNBOOK.md` bölüm 1, 5, 9 | Kerem: SQL onayı, push, test, şablon |
