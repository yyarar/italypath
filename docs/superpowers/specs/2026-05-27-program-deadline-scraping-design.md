...
### Known Per-Target Caveats

Aşağıdaki uni'lerde curated URL belirlerken / pilot scrape sırasında dikkat edilmesi gereken edge case'ler. Bunlar implementation plan'da `Task 11 (pilot scrape)` adımında doğrulanacak:

**University of Trento (universityId: 18)**
- unitn.it'in non-EU öğrenciler için **merkezi bir deadline hub'ı YOK** — tarihler program bazlı yayınlanıyor
- İlk fallback olarak program-specific URL kullanılacak: `https://corsi.unitn.it/en/computer-science/enrollment/admission-non-european-citizens`
- Pilot scrape sırasında şu iki alternatifin daha temsili olup olmadığı doğrulanacak:
  - `https://www.unitn.it/en/international/coming-unitrento/degree-seeking-student`
  - `https://www.unitn.it/en/study/register/deadlines`
- Daha iyi bir merkezi sayfa bulunursa `lib/deadlines/targets.ts`'te Trento entry'sinin URL'i güncellenir
- Pilot bulgusu spec'in altına dipnot olarak eklenir
