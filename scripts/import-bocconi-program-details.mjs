...
const RESULTS_DIR = resolve(
  process.cwd(),
  "bocconi-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "bocconi-program-details-import-report.json");
...
const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  ["economics::bachelor", "economic-and-social-sciences"],
  ["finance::bachelor", "international-economics-and-finance"],
  ["management::bachelor", "international-economics-and-management"],
  ["management and computer science::bachelor", "economics-management-and-computer-science"],
  [
    "management for the arts and cultures::bachelor",
...
async function fetchBocconiDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", BOCCONI_UNIVERSITY_ID)
...
function createPlan(sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: BOCCONI_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages.length > 0 ? source.languages : ["en"],
        duration_years: source.durationYears,
  ...
  return {
    universityId: BOCCONI_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  let mutated = false;
  const existingDepartmentIds = plan.matchedExisting
    .map((department) => department.id)
    .filter((id) => Number.isFinite(id));
  const admissionDetailSnapshot = await snapshotAdmissionDetails(supabase, existingDepartmentIds);
  ...
  try {
    ...
    const refreshedDepartments = await fetchBocconiDepartments(supabase);
...
async function rollbackPlan(supabase, plan, admissionDetailSnapshot) {
  const failures = [];

  const affectedExistingDepartmentIds = plan.matchedExisting
    .map((department) => department.id)
    .filter((id) => Number.isFinite(id));

  if (affectedExistingDepartmentIds.length > 0) {
    const { error } = await supabase
      .from("program_admission_details")
      .delete()
      .eq("university_id", BOCCONI_UNIVERSITY_ID)
      .in("department_id", affectedExistingDepartmentIds);
    if (error) failures.push(`existing admission details cleanup: ${error.message}`);
  }

  if (admissionDetailSnapshot.length > 0) {
    const { error } = await supabase
      .from("program_admission_details")
      .upsert(admissionDetailSnapshot, { onConflict: "department_id" });
    if (error) failures.push(`existing admission details restore: ${error.message}`);
  }
...
function reportForOutput(plan) {
  return {
    ...
    specialChecks: {
      sourceCountExpected: EXPECTED_SOURCE_FILE_COUNT,
      bachelorPrograms: plan.programPlans.filter((programPlan) => programPlan.level === "bachelor"),
      specialProgrammeChoices: plan.programPlans.filter((programPlan) =>
        /double|global experience|cems|asia|concentration|world bachelor/i.test(
          programPlan.canonicalDepartmentName
        )
      ),
      rawLevelNormalized: plan.programPlans.filter((programPlan) => programPlan.rawLevelNormalized),
      uncertainProgramNames: plan.programPlans.filter((programPlan) =>
        programPlan.rawProgramName.toLowerCase().includes("[uncertain]")
      ),
    },
...
async function main() {
  ...
  if (mode === "apply") {
    ...
    console.log(`[OK] Applied ${sourcePrograms.length} Bocconi program details.`);
  } else {
