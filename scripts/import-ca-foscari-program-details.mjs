...
// Usage: node scripts/import-ca-foscari-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "ca-foscari";
const CA_FOSCARI_UNIVERSITY_ID = 9;
const EXPECTED_SOURCE_FILE_COUNT = 29;
const SOURCE_GENERATED_AT = "2026-06-05";
const RESULTS_DIR = resolve(
  process.cwd(),
  "ca-foscari-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "ca-foscari-program-details-import-report.json");
...
const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  [
    "comparative international relations european union studies and global studies",
    "comparative-international-relations",
  ],
  [
    "economics and management of arts and cultural activities economics and administration of arts and culture",
    "economics-and-management-of-arts-and-cultural-activities",
  ],
  ["environmental sciences global change and sustainability", "environmental-sciences"],
  [
    "european american and postcolonial language and literature american studies english studies cultural studies and literary studies",
    "european-american-and-postcolonial-languages-and-literatures",
  ],
  [
    "languages of asia and north africa for business and international cooperation south east asia",
    "languages-of-asia-and-north-africa-for-business-and-international-cooperation",
  ],
  ["language sciences english linguistics and language sciences", "language-sciences"],
  [
    "sustainable chemistry and technologies biomolecular chemistry",
...
async function fetchCaFoscariDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", CA_FOSCARI_UNIVERSITY_ID)
...
function createPlan(sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: CA_FOSCARI_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages.length > 0 ? source.languages : ["en"],
        duration_years: source.durationYears,
  ...
  return {
    universityId: CA_FOSCARI_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchCaFoscariDepartments(supabase);
...
async function main() {
  ...
  const dbDepartments = await fetchCaFoscariDepartments(supabase);
  ...
  if (mode === "apply") {
    ...
    writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
    console.log(`[OK] Applied ${applyResult.detailUpserts} Ca' Foscari program details.`);
  } else {
