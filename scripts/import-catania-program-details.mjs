...
// Usage: node scripts/import-catania-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "catania";
const CATANIA_UNIVERSITY_ID = 57;
const EXPECTED_SOURCE_FILE_COUNT = 11;
const SOURCE_GENERATED_AT = "2026-09-05";
const RESULTS_DIR = resolve(process.cwd(), "catania-english-programs/results");
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "catania-program-details-import-report.json");

const EXCLUDED_SOURCE_FILES = new Map([]);

const FILE_TO_DEPARTMENT = new Map([
  // Medicine & Surgery is mis-seeded as bachelor (a duplicate master row 859
  // also exists — deletion candidate, not touched here); the real programme is
  // a single-cycle LM-41 — level fixed at import.
  ["Medicine_and_Surgery.json", { id: 472, level: "bachelor", expectedFixedLevel: "single-cycle" }],
  ["Agricultural_Sciences_and_Technology.json", { id: 852, level: "master" }],
  ["Chemical_Engineering_for_Industrial_Sustainability.json", { id: 853, level: "master" }],
  ["Construction_Management_and_Safety.json", { id: 854, level: "master" }],
  ["Automation_Engineering_and_Control_of_Complex_Systems.json", { id: 855, level: "master" }],
  ["Communications_Engineering.json", { id: 856, level: "master" }],
  ["Electrical_Engineering_for_Sustainable_Green_Energy_Transition.json", { id: 857, level: "master" }],
  ["Electronic_Engineering.json", { id: 858, level: "master" }],
  ["Data_Science.json", { id: 860, level: "master" }],
  ["Physics.json", { id: 861, level: "master" }],
  ["Global_Politics_and_Euro-Mediterranean_Relations_GLOPEM.json", { id: 863, level: "master" }],
]);

const DEPARTMENT_LEVEL_FIX = { departmentId: 472, from: { level: "bachelor" }, to: { level: "single-cycle", sort_order: 0 } };
...
function extractPrimaryUrl(value, { required = false, field = "", file = "" } = {}) {
  ...
  const official = urls.find((u) => {
    try {
      return new URL(u).hostname.endsWith("unict.it");
    } catch {
      return false;
    }
  });
...
function loadSourceFiles() {
  ...
  if (files.length !== EXPECTED_SOURCE_FILE_COUNT) {
    throw new Error(`Expected ${EXPECTED_SOURCE_FILE_COUNT} Catania source files, found ${files.length}`);
  }
  const skipped = [];
  const sources = [];
...
async function fetchDepartments(supabase) {
  const { data, error } = await supabase
    ...
    .eq("university_id", CATANIA_UNIVERSITY_ID);
  if (error) throw new Error(`Failed to fetch Catania departments: ${error.message}`);
  return data ?? [];
}
...
function buildPlan(sources, skipped, departments) {
  ...
  for (const { file, record, level } of sources) {
    ...
    if (newSpec) {
      ...
      departmentInserts.push({
        file,
        insert: { university_id: CATANIA_UNIVERSITY_ID, ...newSpec, sort_order: maxSortOrderByLevel[newSpec.level] },
      });
  ...
  return {
    universityId: CATANIA_UNIVERSITY_ID,
...
function toDetailPayload(record, departmentId, file) {
  ...
  return {
    ...
    university_id: CATANIA_UNIVERSITY_ID,
  ...
  try {
    if (plan.levelFixNeeded) {
      const { error } = await supabase
        ...
        .eq("university_id", CATANIA_UNIVERSITY_ID);
      if (error) throw new Error(`Failed to fix Catania Medicine level: ${error.message}`);
      levelFixed = true;
    }
    ...
    if (existing.length > 0) throw new Error(`Refusing: ${existing.length} admission rows already exist for Catania.`);
...
async function main() {
  ...
  if (mode === "apply") {
    ...
    writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
    console.log(`[OK] Applied ${applyResult.detailInserts} Catania program details.`);
  } else {
