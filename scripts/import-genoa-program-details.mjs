...
// Usage: node scripts/import-genoa-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "genoa";
const GENOA_UNIVERSITY_ID = 20;
const EXPECTED_IMPORT_COUNT = 24;
const SOURCE_GENERATED_AT = "2026-06-10";
const RESULTS_DIR = resolve(
  process.cwd(),
  "university-of-genoa-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "genoa-program-details-import-report.json");
...
const EXCLUDED_SOURCE_FILES = new Set([
  "Medicine_and_Surgery.json",
  "Chemical_and_Processes_Engineering.json",
  "Civil_Engineering.json",
]);

const DUPLICATE_DEPARTMENT_IDS = new Set([384]);

const SOURCE_OVERRIDES = new Map([
  [
    "Sustainable_Polymer_and_Process_Chemistry.json",
    {
      departmentName: "Sustainable Polymer and Process Chemistry (SMART)",
      slug: "sustainable-polymer-and-process-chemistry",
    },
  ],
  [
    "Digital_Humanities_Interactive_Systems_and_Digital_Media.json",
    {
      departmentName: "Digital Humanities",
      slug: "digital-humanities",
    },
  ],
  [
    "International_Relations_Security_and_International_Relations.json",
    {
...
async function fetchGenoaDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", GENOA_UNIVERSITY_ID)
...
function createPlan(sourcePrograms, skipped, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: GENOA_UNIVERSITY_ID,
  ...
  return {
    universityId: GENOA_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchGenoaDepartments(supabase);
...
async function main() {
  ...
  const dbDepartments = await fetchGenoaDepartments(supabase);
  ...
  if (mode === "apply") {
    ...
    writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
    console.log(`[OK] Applied ${applyResult.detailUpserts} University of Genoa program details.`);
  } else {
