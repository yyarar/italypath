...
// Usage: node scripts/import-milan-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "milan";
const MILAN_UNIVERSITY_ID = 10;
const EXPECTED_SOURCE_FILE_COUNT = 45;
const SOURCE_GENERATED_AT = "2026-06-05";
const RESULTS_DIR = resolve(
  process.cwd(),
  "university-of-milan-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "milan-program-details-import-report.json");
...
const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  ["international medical school ims", "medicine-and-surgery"],
  ["international relations rel", "international-relations-rel-2-curricula-in-english"],
  [
    "management of human resources people data and intelligent technologies",
    "management-of-human-resources-mhr",
  ],
  ["pharmaceutical biotechnology", "pharmaceutical-biotechnology-1-curriculum-in-english"],
  ["philosophical sciences", "philosophical-sciences-1-curriculum-in-english"],
  [
    "public and corporate communication com",
    "public-and-corporate-communication-com-2-curricula-in-english",
  ],
  ["quantitative biology", "nicel-biyoloji-quantitative-biology"],
]);
...
async function fetchMilanDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", MILAN_UNIVERSITY_ID)
...
function createDepartmentCorrection(source, department, dbDepartments, usedSlugs, shouldRename) {
  const to = {
    name: shouldRename ? source.departmentName : department.name,
    slug: shouldRename
      ? desiredSlugForExisting(source, department, dbDepartments, usedSlugs)
      : department.slug,
...
function createPlan(sourcePrograms, dbDepartments) {
  ...
  const warnings = [];
  const detailDepartmentIds = new Map();
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      if (detailDepartmentIds.has(department.id)) {
        warnings.push(
          `Multiple source programs map to department ${department.id}: ${detailDepartmentIds.get(
            department.id
          )} and ${source.file}`
        );
      }
      detailDepartmentIds.set(department.id, source.file);

      const correction = createDepartmentCorrection(
        source,
        department,
        dbDepartments,
        usedSlugs,
        action === "alias-correction"
      );
      if (correction) {
      ...
      department = {
        id: null,
        university_id: MILAN_UNIVERSITY_ID,
  ...
  return {
    universityId: MILAN_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchMilanDepartments(supabase);
...
function reportForOutput(plan, applyResult = null) {
  return {
    ...
    specialChecks: {
      artificialIntelligence: plan.programPlans.filter(
        (programPlan) => programPlan.normalizedName === "artificial intelligence"
      ),
      computerScience: plan.programPlans.filter(
        (programPlan) => programPlan.normalizedName === "computer science"
      ),
      internationalMedicalSchool: plan.programPlans.filter((programPlan) =>
        programPlan.normalizedName.includes("international medical school")
      ),
      quantitativeBiology: plan.programPlans.filter(
        (programPlan) => programPlan.normalizedName === "quantitative biology"
      ),
...
async function main() {
  ...
  const dbDepartments = await fetchMilanDepartments(supabase);
  ...
  if (mode === "apply") {
    ...
    writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
    console.log(`[OK] Applied ${applyResult.detailUpserts} Milan program details.`);
  } else {
