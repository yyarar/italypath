...
// Usage: node scripts/import-padua-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "padua";
const PADUA_UNIVERSITY_ID = 4;
const EXPECTED_SOURCE_FILE_COUNT = 71;
const SOURCE_GENERATED_AT = "2026-06-04";
const RESULTS_DIR = resolve(
  process.cwd(),
  "university-of-padua-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "padua-program-details-import-report.json");
...
const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  [
    ...
    "onay-bekleniyor-applied-child-and-adolescent-psychology-new-programme-2025-26-to-be-approved",
  ],
  [
  ...
  [
    ...
    "management-engineering-study-track-held-in-english-of-the-master-s-degree-in-ingegneria-gestionale",
  ],
  [
    "mechanical engineering for product innovation",
    "onay-bekleniyor-mechanical-engineering-for-product-innovation-new-programme-2025-26-to-be-approved",
  ],
  [
    "mechatronics engineering",
    "onay-bekleniyor-mechatronics-engineering-new-programme-2025-26-to-be-approved",
  ],
  [
    "social sciences and digital data",
    "onay-bekleniyor-social-sciences-and-digital-data-new-programme-2025-26-to-be-approved",
  ],
]);
...
async function fetchPaduaDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", PADUA_UNIVERSITY_ID)
...
function createPlan(sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: PADUA_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages.length > 0 ? source.languages : ["en"],
        duration_years: source.durationYears,
  ...
  return {
    universityId: PADUA_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchPaduaDepartments(supabase);
...
function reportForOutput(plan) {
  return {
    ...
    specialChecks: {
      medicine: plan.programPlans.filter((programPlan) =>
        programPlan.normalizedName.includes("medicine and surgery")
      ),
...
async function main() {
  ...
  const dbDepartments = await fetchPaduaDepartments(supabase);
  ...
  if (mode === "apply") {
    ...
    console.log(`[OK] Applied ${sourcePrograms.length} Padua program details.`);
  } else {
