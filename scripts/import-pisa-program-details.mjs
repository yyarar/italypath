...
const RESULTS_DIR = resolve(process.cwd(), "pisa-english-programmes/results");
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "pisa-program-details-import-report.json");
...
const SOURCE_OVERRIDES = new Map([
  ["iph-bachelor.json", { departmentName: "International Programme in Humanities (IPH)" }],
  ["mbe-l-bachelor.json", { departmentName: "Management for Business and Economics" }],
  ["wia-lm-aerospace.json", { departmentName: "Aerospace Engineering" }],
  ["wai-lm-ai-data-engineering.json", { departmentName: "Artificial Intelligence and Data Engineering" }],
  [
    "wbh-lm-biotech-ai-health.json",
    { departmentName: "Biotechnologies and Applied Artificial Intelligence for Health" },
  ],
  ["wct-lm-communications-engineering.json", { departmentName: "Communications Engineering" }],
  ["wcn-lm-computer-engineering.json", { departmentName: "Computer Engineering" }],
  ["wif-lm-computer-science.json", { departmentName: "Computer Science" }],
  ["wcy-lm-cybersecurity.json", { departmentName: "Cybersecurity" }],
  [
    "wdb-lm-data-science-business-informatics.json",
    { departmentName: "Data Science and Business Informatics" },
  ],
  [
    "wdc-lm-digital-intelligence-change-management.json",
    { departmentName: "Digital Intelligence and Change Management" },
  ],
  ["wes-lm-economics.json", { departmentName: "Economics" }],
  ["wtc-lm-paper-cardboard-engineering.json", { departmentName: "Engineering of Paper and Cardboard" }],
  ["wgf-lm-exploration-applied-geophysics.json", { departmentName: "Exploration and Applied Geophysics" }],
  ["wdh-lm-informatics-digital-health.json", { departmentName: "Informatics for Digital Health" }],
  ["wnt-lm-materials-nanotechnology.json", { departmentName: "Materials and Nanotechnology" }],
  ["wnc-lm-neuroscience.json", { departmentName: "Neuroscience" }],
  ["wsn-lm-nuclear-engineering.json", { departmentName: "Nuclear Engineering" }],
]);

const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  [
    identityKey("Biotechnologies and Applied Artificial Intelligence for Health", "master"),
    "biotechnologies-and-applied-a-i-for-health-wbh-lm",
  ],
  [
    identityKey("Engineering of Paper and Cardboard", "master"),
    "technology-and-production-of-paper-and-cardboard-wtc-lm",
  ],
]);
...
function canonicalDepartmentName(value) {
  return collapseWhitespace(value)
    .replace(/^master'?s?\s+(degree|programme|program)\s+in\s+/i, "")
    .replace(/^master\s+of\s+science\s+in\s+/i, "")
    .replace(/\s+[–-]\s+[A-Z0-9]+-(?:L|LM)\s*$/i, "")
    .replace(/\s*\[[^\]]+\]\s*$/, "")
    .trim();
}
...
function loadSourcePrograms() {
  ...
  return files.map((file) => {
    ...
    for (const field of ["source_quotes", "uncertain"]) assertArray(record, field, file);
...
async function fetchPisaUniversity(supabase) {
  const { data, error } = await supabase
    .from("universities")
    .select("id,name,city,sort_order")
    .eq("id", PISA_UNIVERSITY_ID)
    .single();

  if (error) throw new Error(`Failed to fetch University of Pisa row: ${error.message}`);
  if (!data || data.name !== "University of Pisa" || data.city !== "Pisa") {
    throw new Error(
      `Refusing to import: expected university ${PISA_UNIVERSITY_ID} to be University of Pisa in Pisa, got ${data?.name ?? "missing"} in ${data?.city ?? "missing"}.`
    );
  }
...
async function fetchPisaDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", PISA_UNIVERSITY_ID)
...
function createPlan(university, sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    const validAliasCandidate = aliasCandidate?.level === source.level ? aliasCandidate : undefined;
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: PISA_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages,
        duration_years: source.durationYears,
  ...
  return {
    university,
    universityId: PISA_UNIVERSITY_ID,
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchPisaDepartments(supabase);
...
async function main() {
  ...
  console.log(
    [
      `Mode: ${mode}`,
      `University: ${university.name} (${PISA_UNIVERSITY_ID})`,
