...
// Usage: node scripts/import-polito-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "polito";
const REQUESTED_POLITO_UNIVERSITY_ID = 4;
const POLITO_UNIVERSITY_ID = 5;
const EXPECTED_SOURCE_FILE_COUNT = 31;
const SOURCE_GENERATED_AT = "2026-06-04";
const RESULTS_DIR = resolve(
  process.cwd(),
  "politecnico-di-torino-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "polito-program-details-import-report.json");
...
const SOURCE_TO_EXISTING_SLUG_ALIASES = new Map([
  [identityKey("Cybersecurity Engineering", "master"), "cybersecurity-engineering"],
  [identityKey("Materials Engineering for Industry 4.0", "master"), "materials-engineering-for-industry-4-0"],
  [identityKey("ICT Engineering for Smart Societies", "master"), "ict-engineering-for-smart-societies"],
]);
...
function normalizeLanguages(rawTeachingLanguage) {
  ...
  if (text.includes("multilingual")) {
    languages.push("it", "en");
  } else {
...
function normalizeDocumentArray(record) {
  ...
  if (translation) {
    documents.push(`Document translation/authentication: ${translation}`);
  }
...
async function fetchPolitoUniversity(supabase) {
  const { data, error } = await supabase
    .from("universities")
    .select("id,name,city,sort_order")
    .eq("id", POLITO_UNIVERSITY_ID)
    .single();

  if (error) throw new Error(`Failed to fetch Polito university row: ${error.message}`);
  if (!data || data.name !== "Politecnico di Torino") {
    throw new Error(
      `Refusing to import: expected university ${POLITO_UNIVERSITY_ID} to be Politecnico di Torino, got ${data?.name ?? "missing"}.`
    );
  }
...
async function fetchPolitoDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", POLITO_UNIVERSITY_ID)
...
function createPlan(university, sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: POLITO_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages,
        duration_years: source.durationYears,
  ...
  return {
    requestedUniversityId: REQUESTED_POLITO_UNIVERSITY_ID,
    resolvedUniversityId: POLITO_UNIVERSITY_ID,
    university,
...
function toDetailPayload(source, departmentId) {
  return {
    ...
    admission_type: joinTextParts([
      ...
      labelledText("Admission test type", source.raw.admission_test_type),
      labelledText("Applicant route", source.raw.applicant_route),
      labelledText("Admission call round", source.raw.admission_call_round),
      labelledText("Special call", source.raw.special_call_flag),
    ...
    academic_requirements: joinTextParts([
      ...
      labelledText("Double degree or Erasmus Mundus", source.raw.double_degree_or_erasmus_mundus),
    ]),
    language_requirements: joinTextParts([
      optionalText(source.raw.language_requirements),
      labelledText("English certificate codes", source.raw.english_certificate_codes),
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchPolitoDepartments(supabase);
...
function reportForOutput(plan, applyResult = null) {
  return {
    requestedUniversityId: plan.requestedUniversityId,
    resolvedUniversityId: plan.resolvedUniversityId,
    ...
    specialChecks: {
      bachelorExamples: plan.programPlans.filter((programPlan) => programPlan.level === "bachelor"),
      masterExamples: plan.programPlans.filter((programPlan) => programPlan.level === "master").slice(0, 8),
      newDepartments: plan.programPlans.filter(
        (programPlan) => programPlan.action === "new-department"
      ),
      departmentCorrections: plan.departmentCorrections.map((correction) => ({
        id: correction.id,
      ...
      notCompleteEnglishTrack: plan.programPlans.filter(
        (programPlan) => programPlan.hasCompleteEnglishTrack === false
      ),
...
async function main() {
  ...
  const university = await fetchPolitoUniversity(supabase);
  const sourcePrograms = loadSourcePrograms();
  const dbDepartments = await fetchPolitoDepartments(supabase);
  ...
  if (mode === "apply") {
    ...
    writeFileSync(REPORT_PATH, `${JSON.stringify(report, null, 2)}\n`);
    console.log(`[OK] Applied ${applyResult.detailUpserts} Polito program details.`);
  } else {
