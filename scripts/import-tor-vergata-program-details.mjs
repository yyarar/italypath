...
// Usage: node scripts/import-tor-vergata-program-details.mjs [--dry-run | --apply --project-ref kskbnxxyviowmrlskwke]
const SCRIPT_KEY = "tor-vergata";
const TORVERGATA_UNIVERSITY_ID = 13;
const EXPECTED_IMPORT_COUNT = 23;
const SOURCE_GENERATED_AT = "2026-06-14";
const RESULTS_DIR = resolve(
  process.cwd(),
  "tor-vergata-english-program-admission-requirements/results"
);
const OUTPUT_DIR = resolve(process.cwd(), "output");
const REPORT_PATH = resolve(OUTPUT_DIR, "tor-vergata-program-details-import-report.json");
...
// SOURCE_OVERRIDES maps each source JSON file to the canonical department name
// already present in the DB (or — for new departments — the canonical we want
// stored). Using the DB's canonical name lets the importer match through
// normalizeName() without needing a slug-level alias for that source.
const SOURCE_OVERRIDES = new Map([
  // Single-cycle / level-correction case (alias kept below for level mismatch handling)
  [
    "medicine-and-surgery-md.json",
    { departmentName: "Medicine and Surgery", slug: "medicine-and-surgery" },
  ],
  // New department (Tor Vergata-only English single-cycle 5 years)
  [
    "pharmacy-single-cycle.json",
    { departmentName: "Pharmacy", slug: "pharmacy", durationYears: 5 },
  ],
  // Sources whose program_name string contains parentheticals / curriculum descriptors —
  // canonical to the bare degree name as stored in the DB.
  [
    "business-administration-and-economics-bsc.json",
    { departmentName: "Business Administration & Economics" },
  ],
  [
    "business-administration-msc.json",
  ...
  [
    "management-engineering-msc.json",
    { departmentName: "Management Engineering" },
  ],
  [
    "physics-of-complex-systems-and-big-data-msc.json",
    { departmentName: "Physics of Complex Systems and Big Data" },
  ],
  [
    "physics-of-fundamental-interactions-and-experimental-techniques-msc.json",
    { departmentName: "Physics of Fundamental Interactions and Experimental Techniques" },
  ],
  [
    "clinical-psychosexology-msc.json",
    { departmentName: "Clinical Psychosexology" },
  ],
  [
    "chemistry-for-nano-engineering-msc.json",
    { departmentName: "Chemistry for Nano-Engineering" },
  ],
  [
    "astrophysics-and-space-science-msc.json",
    { departmentName: "Astrophysics" },
  ],
  [
    "scienze-della-storia-e-del-documento-msc.json",
    { departmentName: "European History - Scienze della Storia e del Documento" },
  ],
  [
    "innovation-tourism-for-made-in-italy-bsc.json",
    { departmentName: "Tourism Sciences" },
  ],
  // Keep DB name as-is for ambiguous cases. The source program_name
  // ("Sport and Health Promotion") may be the actual official English name,
  // but we conservatively keep the existing DB name pending coordinator confirmation.
  [
    "sport-and-health-promotion-msc.json",
    { departmentName: "Physical Activity and Health Promotion" },
  ],
]);

// The only remaining alias is for Medicine and Surgery: the DB has it stored
// under level "bachelor" while our source identifies it as "single-cycle".
...
function normalizeRequiredDocuments(value) {
  // Tor Vergata source: required_documents is a free-text string (per fields.yaml output_shape).
  // Some entries are semi-colon / bullet / newline separated; some are a single paragraph.
  // We split on common list separators and trim, then keep non-empty entries.
  if (Array.isArray(value)) {
    return value.flatMap((item) => {
      const normalized = optionalText(item);
      return normalized ? [normalized] : [];
  ...
  // Split on bullet markers or newlines, fall back to semi-colons.
  const lines = trimmed
    .split(/\r?\n|•|·|^\s*[-*]\s+/m)
    .map((line) => line.trim())
    .filter(Boolean);
...
function loadSourcePrograms() {
  ...
  return files.map((file) => {
    ...
    const durationYears = override?.durationYears ?? defaultDurationForLevel(level);
...
async function fetchTorVergataUniversity(supabase) {
  const { data, error } = await supabase
    .from("universities")
    .select("id,name,city,sort_order")
    .eq("id", TORVERGATA_UNIVERSITY_ID)
    .single();

  if (error) throw new Error(`Failed to fetch Tor Vergata university row: ${error.message}`);
  if (!data || data.name !== "University of Rome Tor Vergata") {
    throw new Error(
      `Refusing to import: expected university ${TORVERGATA_UNIVERSITY_ID} to be 'University of Rome Tor Vergata', got '${data?.name ?? "missing"}'.`
    );
  }
...
async function fetchTorVergataDepartments(supabase) {
  return fetchAllRows(
    supabase,
    ...
    (query) =>
      query
        .eq("university_id", TORVERGATA_UNIVERSITY_ID)
...
function createPlan(university, sourcePrograms, dbDepartments) {
  ...
  for (const source of sourcePrograms) {
    ...
    if (department) {
      ...
      department = {
        id: null,
        university_id: TORVERGATA_UNIVERSITY_ID,
        name: source.departmentName,
        slug,
        languages: source.languages,
        duration_years: source.durationYears,
  ...
  return {
    university,
    universityId: TORVERGATA_UNIVERSITY_ID,
...
function toDetailPayload(source, departmentId) {
  ...
  return {
    ...
    admission_type: joinTextParts([
      optionalText(raw.admission_type),
      labelledText("Selection type", raw.selection_type),
      labelledText("Admission test (name and provider)", raw.admission_test_name_and_provider),
      labelledText("EU / non-EU quota", raw.eu_vs_non_eu_quota),
      labelledText("School", raw.school),
    ]),
    academic_requirements: optionalText(raw.academic_requirements),
    language_requirements: joinTextParts([
      optionalText(raw.language_requirements),
      labelledText("English proficiency minimum scores", raw.english_proficiency_minimum_scores),
      labelledText("Italian B1 during studies", raw.italian_b1_requirement_during_studies),
    ]),
    application_deadline_eu: joinTextParts([
      optionalText(raw.application_deadline_eu),
      labelledText("Rolling vs window", raw.rolling_vs_window_admission),
      labelledText("Tuition fee band (ISEE / flat)", raw.tuition_fee_band_isee_or_flat),
      labelledText("Application fee and enrolment deposit", raw.application_fee_and_enrolment_deposit),
      labelledText("Scholarships at admission time", raw.scholarship_links_at_admission_time),
    ]),
    application_deadline_non_eu: joinTextParts([
      optionalText(raw.application_deadline_non_eu),
      labelledText("Non-EU pre-enrolment portal (Universitaly)", raw.non_eu_pre_enrolment_portal),
      labelledText("DoV / CIMEA statement required", raw.dov_or_cimea_statement_required),
    ]),
    ...
    official_call_url: joinTextParts([
      optionalText(raw.official_call_url),
      labelledText("Call for Applications PDF", raw.call_for_applications_pdf_url),
      labelledText("Double / joint degree partners", raw.double_or_joint_degree_partner_universities),
    ]),
...
async function applyPlan(supabase, plan) {
  ...
  try {
    ...
    const refreshedDepartments = await fetchTorVergataDepartments(supabase);
...
async function main() {
  ...
  const university = await fetchTorVergataUniversity(supabase);
  const dbDepartments = await fetchTorVergataDepartments(supabase);
  ...
  console.log(
    [
      `Mode: ${mode}`,
      `University: ${university.name} (${TORVERGATA_UNIVERSITY_ID})`,
